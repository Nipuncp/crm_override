...
function fetchCallLogs() {
  ...
  list.value = createResource({
    ...
    onSuccess(data) {
      ...
      totalRecords.value = data.total_count
      defaultParams.value = {
        doctype: props.doctype,
  ...
  // Wait for a short time after createResource call
  setTimeout(() => {
    list.value.params = list.value.params ? list.value.params : getParams()
  }, 500) // Adjust timeout as needed (300ms-1000ms depending on debounce/fetch speed)
}
