...
def get_quick_filters(doctype: str, cached: bool = True):
    ...
    if global_settings := frappe.db.exists(
        "CRM Global Settings", {"dt": doctype, "type": "Quick Filters"}
    ):
        _quick_filters = frappe.db.get_value(
            "CRM Global Settings", global_settings, "json"
        )
    ...
    if doctype == "CRM Lead":
        quick_filters = [
            filter for filter in quick_filters if filter.get("fieldname") != "converted"
        ]
...
def create_update_global_settings(doctype, quick_filters):
    if global_settings := frappe.db.exists(
        ...
        frappe.db.set_value(
            "CRM Global Settings", global_settings, "json", json.dumps(quick_filters)
        )
    else:
...
def get_data(
    ...
    if view_type != "kanban":
        ...
        if doctype == "CRM Call Log":
            # Extract page and limit from the request
            page = int(frappe.form_dict.get("page", 1))  # Default to page 1
            limit = int(
                frappe.form_dict.get("limit", 20)
            )  # Default to 20 records per page
            # Calculate limit_start
    ...
    if view_type == "kanban":
        ...
        for kc in kanban_columns:
            ...
            order = kc.get("order")
            if (
                column_field in filters and filters.get(column_field) != kc.get("name")
            ) or kc.get("delete"):
                column_data = []
            else:
        ...
        for field in fields:
            if field.get("fieldname") == group_by_field:
                group_by_field = {
                    ...
                    "fieldtype": field.get("fieldtype"),
                    "options": get_options(
                        field.get("fieldtype"), field.get("options")
                    ),
                }
...
@frappe.whitelist()
def get_fields_meta(
    doctype, restricted_fieldtypes=None, as_array=False, only_required=False
):
